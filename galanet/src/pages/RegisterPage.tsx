    ...
    return (
        <div className="w-full flex justify-center">
            <form className="bg-slate-800 w-full sm:w-4/5 md:w-3/5 lg:w-2/5 h-fit text-slate-50 py-6 px-6 sm:py-8 sm:px-10 md:py-10 md:px-16 rounded-lg grid gap-6">
                <h1 className="text-2xl text-center font-bold">Passe-droit</h1>
                <div className="grid gap-1">
                    <p className="text-sm sm:text-base">Alias de Contrebandier</p>
                    <input type="text" className="w-full flex items-center justify-center bg-slate-700 border border-slate-400 rounded-lg p-2.5 sm:p-3 outline-none text-sm sm:text-base"/>
                </div>
                <div className="grid gap-1">
                ...
                <div className="grid gap-1">
                    <p className="text-sm sm:text-base">Confirme ta clé de cryptage</p>
                    <div className='flex items-center w-full justify-between bg-slate-700 border border-slate-400 rounded-lg p-2.5 sm:p-3'>
                        <input type={showConfirmPassword ? "text" : "password"} className="w-full outline-none bg-transparent text-sm sm:text-base"/>
                        <div onClick={() => setShowConfirmPassword(!showConfirmPassword)} className='cursor-pointer ml-2'>
                            { showConfirmPassword ? <EyeOff className="w-5 h-5 sm:w-6 sm:h-6" /> : <ScanEye className="w-5 h-5 sm:w-6 sm:h-6"/>}
                        </div>
                    </div>
                </div>
                <Button text="Générer l'identifiant" to="/" fullWidth/>
                <p className='text-center text-xs sm:text-sm'>Vous avez un compte ? <Link to="/login" className='font-bold hover:underline'>Connexion</Link></p>
            </form>
        </div>
    )
}
